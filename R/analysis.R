...
map_prep <- function(otu_table, enviroment_data, ukcoast_poly, ukcoast_line){
    ...
    ## Terra wants long form explicit grid info, AI generated need to check.
    
    ukgrid = "+proj=tmerc +lat_0=49 +lon_0=-2 +k=0.9996012717 +x_0=400000 +y_0=-100000 +ellps=airy +towgs84=446.448,-125.157,542.06,0.15,0.247,0.842,-20.489 +units=m +no_defs"
    ...
    ## Replaced above with belwo which are atempts to move to terra - generated by AI, need to check
    terra::crs(uk.poly) <- latlong
    terra::crs(uk.line) <- ukgrid    
    uk.poly <- terra::project(uk.poly, ukgrid)
    uk.line <- terra::project(uk.line, ukgrid)
    ...
    ## Replaced above with terra, AI generated code
    g <- as.data.frame(uk.poly)
    x.range <- as.integer(range(terra::crds(uk.poly)[,1]))
    y.range <- as.integer(range(terra::crds(uk.poly)[,2]))
